<template>
  <div v-bind:class="fag?'items':'items-change'" @click="toMarkDetail">
    <div class="itemtop">
      ...
      <div class="text" v-show="fag1">
        <div class="name">
          <div>
            <span>{{itemdata.regNum}}</span>
          </div>
          <div>
            <span>第{{itemdata.intCls}}类-{{itemdata.intClsName}}</span>
          </div>
        </div>
        <div class="itemcontent" v-show="fag">
          <div class="intClsName">
            <span>第{{itemdata.intCls}}类-{{itemdata.intClsName}}</span>
          </div>
          <div>
...
export default {
  ...
  methods: {
    ...
    toMarkDetail() {
      //由于要解决：从详情页 返回列表页的数据会刷新问题，必须用嵌套路由，及在统一组件内控制加载
      this.$router.push({path:this.$route.path+'/markdetail',query:{regNum:this.$options.propsData.itemdata.regNum,intCls:this.$options.propsData.itemdata.intCls}})
    }
  },
  mounted: function() {
