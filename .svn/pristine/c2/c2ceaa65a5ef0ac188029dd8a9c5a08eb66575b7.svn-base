<template>
  <div>
    ...
    <div class="graypanel" v-show="isOpenSimilar">
      <div class="propanel">
        <ul>
          <li :class="{'active':index===nowSelected1, 'active2':goodsSimilarHaveChecked.indexOf(index+1)!=-1}" v-for="(item, index) in goodsList" @click.stop="selectGoods(item.id,index)" :key='index'>
      ...
      <div class="regionpanel">
        <ul>
          <!-- <li>不限</li> -->
          <li style="overflow: hidden;" v-for="(item, index) in erGoodsList" :key="index">
    ...
    <div class="graypanel" v-show="isOpenGoods">
      <div class="content">
        ...
        <serviceItem v-for="(item,index) in goodsItems1" :itemFirst="item" :key="index"></serviceItem>
      </div>
...
export default {
  ...
  methods: {
    ...
    getSimCertain() {
      this.isOpenSimilar = false
      let params = {
        secondClassIndustryList: this.goodsSimilar,
        isCrossIndex: this.ifCrossSearch
      }
      this.$emit('getSimId', params)
    },
    // 类似商品/服务 重置
    resetSim() {
      this.goodsSimilar = []
