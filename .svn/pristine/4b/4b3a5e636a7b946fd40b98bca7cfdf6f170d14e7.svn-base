<template>
  <div class="container">
    ...
    <div style="padding: 0px .76rem;position: relative;top: -113px;margin-bottom: -65px;">
      <div class="iconmenu">
        <div class="icon-content" @click="gotoSearch('goods')">
          <img src="../../assets/image/shangpinchaxun@3x.png">
          <div>商品查询</div>
        </div>
        <div class="icon-content">
          <img src="../../assets/image/hangyechaxun@3x.png">
          <div>行业查询</div>
        </div>
      ...
      <div class="newsbar">
        <span>资讯阅读</span>
        <span class="more" @click="policyServices()">更多</span>
      </div>
      <div class="newslist">
...
export default {
  ...
  methods: {
    ...
    policyService(newsType) {
      this.$router.push({ path: 'newsList', query: { newsType: newsType } })
    },
    policyServices() {
    ...
    getNew() {
      ...
      api.getFirstNew(propsData).then((res) => {
  ...
  mounted: function() {
    ...
    if (localStorage.mark_mobile && localStorage.mark_mobile != 'undefined' && localStorage.mark_mobile != '') {
      ...
      if (window.localStorage.nearMultipleSearch) {
        let post1 = {
          keywordType: "4",
          phoneId: localStorage.mark_mobile,
          keywords: window.localStorage.nearMultipleSearch.split(',')
        }
        api.saveNearSearch(post1).then((res) => { })
      } else {
        localStorage.setItem('nearMultipleSearch', '');
      }
