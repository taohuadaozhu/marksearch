  ...
  <div class="container">
    ...
    <div class="detail-one" ref="detailone" :class="{ 'class-a': isA, 'class-b': isB}">
      <div class="top-two" v-for="item in getTitltContent" style="z-index:0;width:100%">
        <div class="title"><span>第{{item.sort}}类</span> {{item.title}}</div>
    ...
    <div class="note" v-for="item in getTitltContent">
      <!-- 注释 -->
      <div class="annotation" v-html="item.note" style="font-size: .9rem;margin:.5rem 0.77rem;background:white;padding:0 0.8rem;box-shadow:0px 10px 10px 0px #f2f2f2;"></div>

      <div class="graypanel">
        <div class="propanel">
          <ul style="margin-bottom:4rem">
            <li v-for="(item,index) in ErAllTitle" :class="{'active1':index===nowSelected1}" @click.stop="selectPro(item.id, index)">【{{item.similar_code}}】</li>
          </ul>
        </div>
        <!-- 信息 -->
        <div class="regionpanel">
          <span v-for="item in dataTitltContent">{{item.title}}</span>
...
<script>
// var liFirst = document.getElementsByTagName("li")[0];
// console.log(liFirst)
  ...
  methods: {
    ...
    getEr() {
      api.getErAllTitle(this.$route.query.id).then((res) => {
        ...
        this.selectPro(this.ErAllTitle[0].id, 0)
      })
    },
    getTitlt() {
...
.container {
  ...
  .detail-one {
    ...
    .top-two {
      height: 100%;
      float: left;
      padding: .77rem .77rem .5rem .77rem;
      position: relative;
      box-sizing: border-box;
      .title {
        font-size: 1.38rem;
        font-weight: 600;
        background: linear-gradient(90deg, #9329DB,#0A195A);
        color: #fff;
        height: 4.61rem;
        line-height: 4.61rem;
        border-radius: 5px;
...
.explain div {
  font-size: 1rem; // padding: 0rem .5rem;
  color: white;
}
